...
df <- as.data.frame(nycflights)
sfo_feb_flights <- subset(df, dest == "SFO" & month == 2)
```
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(mean_dd = mean(arr_delay), sd_dd = sd(arr_delay), n = n(), IQR = IQR(arr_delay))
...
nycflights %>%
  group_by(month) %>%
  summarise(median_dp = median(dep_delay)) %>%
  arrange(desc(median_dp))
```
